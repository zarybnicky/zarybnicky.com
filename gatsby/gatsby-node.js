...
const basePath = path.resolve(process.env.VAULT_LOCATION);
...
exports.createPages = async ({ graphql, actions }) => {
  ...
  for (let i = 0; i < result.data.allMarkdownRemark.edges.length; i++) {
    ...
    const outgoingLinks = findReferences(node.rawMarkdownBody) // All outgoing links from this note
  ...
  for (let i = 0; i < result.data.allMarkdownRemark.edges.length; i++) {
    ...
    for (let j = 0; j < linkedNoteTitles.length; j++) {
      ...
      // This reduces the page context size. Use only things used in note.tsx. Otherwise I would have just set it as `node`.
      ...
      linkedNotes[linkTitle.toLowerCase()] = {
        ...
        html: allNotesByTitle[linkTitle].html
      }
    }
  ...
  createPage({
    path: `/note-map`,
    component: path.resolve(`./src/templates/note-map.tsx`),
    context: {
  ...
  if (process.env.NODE_ENV !== 'production') {
    const privateNotes = await graphql(`
      query {
        allMarkdownRemark(filter: { fields: { visibility: { eq: "unlisted" } } }) {
          edges {
            node {
              fields {
                slug
    ...
    `)
    for (let i = 0; i < privateNotes.data.allMarkdownRemark.edges.length; i++) {
      const node = privateNotes.data.allMarkdownRemark.edges[i].node
