...
export const DefaultMenuStructure = (menuType = 'main') => {
  ...
  const { site, allMarkdownRemark } = useStaticQuery(
    graphql`
      query {
        site {
          siteMetadata {
            menu { type item title }
            headerMenu { type item title }
          }
        }
  ...
  if (menuType === 'main') {
    if (site.siteMetadata.menu !== undefined) {
      structure = site.siteMetadata.menu
    } else if (site.siteMetadata.headerMenu !== undefined) {
      // If not, use the header menu.
      structure = site.siteMetadata.headerMenu
    }
  } else if (menuType === 'header') {
    if (site.siteMetadata.headerMenu !== undefined) {
      structure = site.siteMetadata.headerMenu
    } else if (site.siteMetadata.menu !== undefined) {
      structure = site.siteMetadata.menu
    }
  } else if (menuType === 'tag-list') {
    let tagList = allMarkdownRemark.group
      .sort((a, b) => {
...
export function MenuRoot({ menu }) {
  return <ul>{menu.map((x, i) => <MenuItem item={x} key={i} />)}</ul>;
}

export function MenuItem({ item, className }: { item: any; className?: string; }) {
  let itm
  if (item.type === 'page')
